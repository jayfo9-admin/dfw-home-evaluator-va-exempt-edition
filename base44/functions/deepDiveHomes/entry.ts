...
Deno.serve(async (req) => {
  try {
    ...
    // Fire off deep dive async without waiting
    (async () => {
      try {
...
Be forensic and critical. Assume 100% P&T Disabled Veteran buyer.`,
        ...
        console.error('Deep dive batch failed:', err.message);
      }
    })();

    // Return immediately
    return Response.json({ message: 'Deep dive started in background', homeIds: homeIds.length });
  } catch (error) {
